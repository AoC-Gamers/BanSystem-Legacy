...
DROP PROCEDURE IF EXISTS `bansystem_access_ban_save` $$
CREATE PROCEDURE `bansystem_access_ban_save`(
    IN inAccountId INT,
    IN inSteamId64 VARCHAR(20),
    IN inPlayerName VARCHAR(128),
    IN inIpAddress VARCHAR(64),
...
BEGIN
    INSERT INTO `bansystem_access_bans` (
...
DROP PROCEDURE IF EXISTS `bansystem_access_ban_delete` $$
CREATE PROCEDURE `bansystem_access_ban_delete`(
    IN inAccountId INT
)
BEGIN
    DELETE FROM `bansystem_access_bans`
    WHERE `accountid` = inAccountId;

    CALL `bansystem_rebuild_summary_account`(inAccountId);
END $$

DROP PROCEDURE IF EXISTS `bansystem_access_get_active_by_accountid` $$
CREATE PROCEDURE `bansystem_access_get_active_by_accountid`(
    IN inAccountId INT
)
BEGIN
    SELECT
...
DROP PROCEDURE IF EXISTS `bansystem_access_get_active_by_banid` $$
CREATE PROCEDURE `bansystem_access_get_active_by_banid`(
    IN inBanId INT
)
BEGIN
    SELECT
