...
stock void BSComm_QueueInfoByAccountId(int iAdmin, int iAccountId, ReplySource eReplySource = SM_REPLY_TO_CONSOLE)
{
	...
	char szQuery[640];
	int iLen = 0;
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "SELECT `player_name`, `ban_type`, `ban_length`, `ban_reason`, `ban_context`, ");
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "`banned_by`, `banned_by_name`, IFNULL(UNIX_TIMESTAMP(`date_expire`), 0) AS `date_expire_ts` ");
...
stock void BSComm_QueueList(int iAdmin, int iLimit, ReplySource eReplySource = SM_REPLY_TO_CONSOLE)
{
	...
	char szQuery[896];
	int iLen = 0;
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "SELECT `player_name`, `accountid`, `ban_type`, `ban_length`, `ban_reason`, `ban_context`, ");
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "`banned_by`, `banned_by_name`, IFNULL(UNIX_TIMESTAMP(`date_expire`), 0) AS `date_expire_ts` ");
...
stock void BSComm_QueueAddBan(int iAdmin, int iAccountId, int iTargetClient, eBSCommType eCommType, int iLength, const char[] szReason, const char[] szContext, const char[] szSteamId64Override = "", const char[] szPlayerNameOverride = "UNKNOWN", ReplySource eReplySource = SM_REPLY_TO_CONSOLE)
{
	...
	char szQuery[2304];
	int iLen = 0;
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "INSERT INTO `bansystem_comm_bans` ");
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "(`accountid`, `steamid64`, `player_name`, `ip_address`, `ban_type`, `ban_length`, `ban_reason`, ");
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "`ban_context`, `banned_by`, `banned_by_name`, `banned_by_steamid64`) ");
	...
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "ON DUPLICATE KEY UPDATE `steamid64` = VALUES(`steamid64`), `player_name` = VALUES(`player_name`), ");
	iLen += g_dbBSComm.Format(szQuery[iLen], sizeof(szQuery) - iLen, "`ip_address` = VALUES(`ip_address`), `ban_type` = VALUES(`ban_type`), `ban_length` = VALUES(`ban_length`), ");
	Format(szQuery[iLen], sizeof(szQuery) - iLen, "`ban_reason` = VALUES(`ban_reason`), `ban_context` = VALUES(`ban_context`), `banned_by` = VALUES(`banned_by`), `banned_by_name` = VALUES(`banned_by_name`), `banned_by_steamid64` = VALUES(`banned_by_steamid64`);");
...
public void BSComm_OnAddBanCompleted(Database db, DBResultSet rsResult, const char[] szError, any pData)
{
	...
	BSNormalLogToFileEx(g_cvBSLogMode, "[BanSystem Comm]", "enforcement", "action=ban_added accountid=%d type=%d length=%d live_target=%d", iAccountId, view_as<int>(eCommType), iLength, (iLiveTarget > 0 && IsClientInGame(iLiveTarget)) ? 1 : 0);
}
...
public void BSComm_OnRemoveBanCompleted(Database db, DBResultSet rsResult, const char[] szError, any pData)
{
	...
	BSNormalLogToFileEx(g_cvBSLogMode, "[BanSystem Comm]", "enforcement", "action=ban_removed accountid=%d live_target=%d", iAccountId, (iTarget > 0 && IsClientInGame(iTarget)) ? 1 : 0);
...
public void BSComm_OnInfoLoaded(Database db, DBResultSet rsResult, const char[] szError, any pData)
{
	...
	BSComm_GetCommTypeLabel(eCommType, szType, sizeof(szType));
	BSComm_PrintAdminInfoConsoleCard(iAdmin, iAccountId, szPlayerName, szSteam2, szType, iLength, szReason, szContext, szBannedByName, szDateExpire);
...
public void BSComm_OnListLoaded(Database db, DBResultSet rsResult, const char[] szError, any pData)
{
	...
	do
	{
		...
		BSComm_GetCommTypeLabel(eCommType, szType, sizeof(szType));
		BSComm_FormatExpireDisplay(iDateExpireTs, szDateExpire, sizeof(szDateExpire));
		BSComm_PrintAdminListConsoleCard(iAdmin, iAccountId, szPlayerName, szSteam2, szType, iLength, szReason, szContext, szBannedByName, szDateExpire);
